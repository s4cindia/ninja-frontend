...
- **Stylesheet Detection**: Replaced manual citation detection with automatic stylesheet detection at `/editorial/citations/:jobId`. The backend auto-identifies citation style (with confidence score), checks citation number sequences, cross-references body citations against the reference list, and provides format conversion options (APA 7th, MLA 9th, Chicago 17th, Vancouver, IEEE). The UI displays: detected style badge with confidence meter, sequence analysis (missing numbers, gaps, duplicates), cross-reference check (matched/orphaned/uncited), scrollable reference table with match status, and a style conversion dropdown. Upload page no longer requires manual style selection.
